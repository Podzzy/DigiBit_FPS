...
public class WeaponController : MonoBehaviour
{
    ...
    private void NetworkStart()
    {
        ...
        //Get the camera shake if we are the owner as well, no need on non owners as their camera is disabled.
        if (np.networkObject.IsOwner)
        {
            camShake = np.PlayerCamera.GetComponent<CameraShake>();
        }
    }
    ...
    /// <summary>
    /// The RPC for the shooting, called from the event on the networking player
    /// </summary>
